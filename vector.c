...
Point getMaxPoint(Point array[MAX_POINT_NUM]) {
  ...
  for (size_t i = 1; i < MAX_POINT_NUM; i++) {
    if (array[i].x > max.x) max.x = array[i].x;
    if (array[i].y > max.y) max.y = array[i].y;
  }
  return max;
}
...
Point getMinPoint(Point array[MAX_POINT_NUM]) {
  ...
  for (size_t i = 1; i < MAX_POINT_NUM; i++) {
    if (array[i].x < min.x) min.x = array[i].x;
    if (array[i].y < min.y) min.y = array[i].y;
  }
  return min;
}
...
void printPoint(Point p) { printf("(%.2f, %.2f)\n", p.x, p.y); }
